BEGIN {
    ...
    mpiAppPath = ENVIRON["GearsDir"] "/start_boinc";
    ...
    if (tasks4Start > 0 && boincTaskInQueue == 0)
    {
        ...
        while ((getline < tasksListPath > 0) && startedTasks < tasks4Start)
        {
            ...
            if (startFlags[$2] + 0 == 0)
            {
                ...
                startCommand = "mpirun VIADEV_USE_AFFINITY=0 -np " nodesInTask*coresPerNode " -maxtime " catchNodeTime " " mpiAppPath " " taskName " " workPath " " startFlag " " stopFlag " " boincRunTime " " nodesList;
                print(startCommand);
                startedTasks ++;
                system(startCommand);
            }
